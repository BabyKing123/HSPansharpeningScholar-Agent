...
def build_file_fingerprint(pdf_path: str | Path, hash_len: int = 10) -> str:
    """根据 PDF 文件内容生成稳定指纹。

    输入：
        pdf_path: PDF 文件路径。
        hash_len: 返回的 hash 前缀长度。
    输出：
        PDF 文件内容 sha1 hash 的前缀。
    异常：
        当文件不可读取时，抛出对应文件读取异常。
    """
...
def build_document_id(pdf_path: str | Path) -> str:
    ...
    fingerprint = build_file_fingerprint(pdf_path)
    return f"paper_{fingerprint}"
...
def parse_pdf_files(
    ...
    for pdf_path in pdf_paths:
        ...
        try:
            fingerprint = build_file_fingerprint(current_path)
            original_path = seen_fingerprints.get(fingerprint)
                ...
                errors.append(
                    {
                        ...
                        "error_message": (
                            f"内容重复，已跳过：与 {original_path.name} 文件内容一致"
            ...
            seen_fingerprints[fingerprint] = current_path
