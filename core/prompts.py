...
def build_answer_system_prompt() -> str:
    ...
    return (
        ...
        "请优先依据召回到的论文片段作答，不要编造未在论文中出现的结论。"
        "回答时重点关注任务定义、输入模态、空间-光谱信息建模、网络结构、损失函数、退化模型、数据集、评价指标、实验结论和对用户模型的启示。"
        "如果论文不是严格的 hyperspectral pansharpening，请说明它与用户研究方向的距离，例如 MSI pansharpening、HSI-MSI fusion、HSI super-resolution、latent HSI restoration 或 general image fusion。"
    )
...
def build_answer_task_prompt(question: str, context_blocks: list[str]) -> str:
    ...
    return (
        f"用户问题：{question}\n\n"
        f"请仅基于以下来源片段整理回答：\n{context_text}\n\n"
        "输出时优先总结与全色锐化、高光谱全色锐化、HSI-MSI fusion、空间-光谱融合、zero-shot、latent diffusion 或退化建模最相关的信息，并指出证据边界。"
    )
